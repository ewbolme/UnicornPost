...
parser.add_argument('--segmentation_solution_name', type=str, help='segmentation solution name')
parser.add_argument('--personalization_solution_name', type=str, help='Personalization solution name')
parser.add_argument('--personalization_campaign_name', type=str, help='Campaign name personalization')
...
deskdrop_interactions_cluster = "s3://"+bucket_name+"/personalize_ready/deskdrop_interactions_clustered.csv"           ##Need to set the name
...
interactions_create_dataset_import_job_response = personalize_client.create_dataset_import_job(
    ...
    datasetArn = interactions_dataset_arn,
    dataSource = {
        "dataLocation": deskdrop_interactions_cluster
    },
...
while time.time() < max_time:
    ...
    if status == "ACTIVE":
        print(f'Import job {interactions_dataset_import_job_arn} successfully completed')
        break
...
print("\n\nBreaking news clustered solution arn:",json.dumps(user_personalization_solution_arn, indent=2))
...
user_segmentation_solution_arn = user_segmentation_create_solution_response['solutionArn']
print("\n\nBreaking news segmentation solution arn:",json.dumps(user_segmentation_solution_arn, indent=2))
...
print("\n\nBreaking news clustered solution version arn:",json.dumps(user_personalization_solution_version_arn, indent=2))


user_segmentation_create_solution_version_response = personalize_client.create_solution_version(
    solutionArn = user_segmentation_solution_arn
)


user_segmentation_solution_version_arn = user_segmentation_create_solution_version_response['solutionVersionArn']
print("\n\nBreaking news segmentation solution version arn:",json.dumps(user_segmentation_solution_version_arn, indent=2))
...
while time.time() < max_time:
    ...
    status_user_segmentation_solution = user_segmentation_version_response["solutionVersion"]["status"]
    ...
    if status_user_segmentation_solution == 'ACTIVE' and status_user_personalization_solution == "ACTIVE":
        break
