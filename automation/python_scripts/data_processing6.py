...
parser.add_argument('--user_seg_model_name', type=str, help='name of personalize user segmentation model')
...
print("role given:",role,"bucket name given",bucket_name, "user segmentation model name",user_seg,"\njob",job_name)
...
batch_file_name = "s3://"+ bucket_name +"/user_segementation/output/breakingnews_user_segmentation.json"
...
try:
    updated_data = pd.concat([existing_data, pd.DataFrame(output_result)], ignore_index=True)
